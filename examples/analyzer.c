...
int main(int argc, char* argv[])
{
    ...
    enum AicErrorCode sample_rate_result =
    ...
    size_t block_num_frames = 0;
    enum AicErrorCode num_frames_result =
        aic_model_get_optimal_num_frames(model, sample_rate, &block_num_frames);
    if (num_frames_result != AIC_ERROR_CODE_SUCCESS || block_num_frames == 0)
    {
        printf("Failed to get optimal frame count with error: %d\n", num_frames_result);
        aic_model_destroy(model);
        return 1;
    }

    // Analyzer creation: the collector buffers audio, the analyzer processes the latest buffer.
    ...
    enum AicErrorCode initialize_result =
        aic_collector_initialize(collector,
                                 sample_rate,
                                 NUM_CHANNELS,
                                 block_num_frames,
                                 true);
    ...
    size_t signal_num_frames = sample_rate;
    float* signal = (float*)calloc(signal_num_frames, sizeof(float));
    if (!signal)
    {
    ...
    for (size_t offset = 0; offset < signal_num_frames; offset += block_num_frames)
    {
        size_t remaining = signal_num_frames - offset;
        size_t frames_this_block = remaining < block_num_frames ? remaining : block_num_frames;
        enum AicErrorCode buffer_result =
            aic_collector_buffer_sequential(collector,
                                            signal + offset,
                                            NUM_CHANNELS,
                                            frames_this_block);
    ...
    enum AicErrorCode analyze_result =
