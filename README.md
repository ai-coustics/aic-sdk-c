...
aic_collector_initialize(collector, sample_rate, num_channels, num_frames, false);
```

Buffer the audio using the `aic_collector_buffer_*` APIs. See the `Processing Audio` section for more details.
...
See the [`examples/basic.c`](examples/basic.c) and [`examples/analyzer.c`](examples/analyzer.c) files for complete working examples.

To run the basic processor example with the default model:
...
You can pass a custom model to either example:

```bash
make run-basic MODEL_PATH="path/to/your/processor-model.aicmodel"
make run-analyzer MODEL_PATH="path/to/your/analyzer-model.aicmodel"
```

**Note:** If you don't provide `MODEL_PATH`, the Makefile will automatically download the default model for the selected example. Use `make run-examples` to run both examples with their default models; `run-examples` does not accept `MODEL_PATH` because the examples require different model types.
